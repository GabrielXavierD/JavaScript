function processa() {
  ...
  xhttp.onload = function () {
    //atribuindo função anonima que sera chamada quando houver uma resposta para a requisição
    ...
    if (objResponse && objResponse.query && objResponse.query.pages) {
      //verifica se o objeto existe e se são validas algumas propriedades (.query etc...)
      let wikiPage =
        objResponse.query.pages[Object.keys(objResponse.query.pages)[0]]; //inexações para ter acesso a pagina que irá ter o conteudo principal (wikiPage)
      ...
      if (mainText) {
        //se o texto existir e não nulo faça:
        document.getElementById("resposta").innerHTML = mainText; //atribui o texto como conteudo interno da div
      } else {
        //se termo não existir
        document.getElementById("resposta").innerHTML =
          "Não encontrei explicação para " + termoPesquisa; //vai escrever na div
      }
    }
  }; //abrindo requisição com o metodo, a url para consulta e o termo para pesquisa
  xhttp.open(
    "GET",
    "https://pt.wikipedia.org/w/api.php?format=json&origin=*&action=query&prop=extracts&explaintext=1&titles=" +
      termoPesquisa
  );
